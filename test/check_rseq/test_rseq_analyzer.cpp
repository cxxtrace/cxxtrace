...
TEST(test_rseq_analyzer_x86, empty_function_is_disallowed)
{
  ...
  EXPECT_THAT(analysis.problems(),
              ElementsAre(VariantWith<rseq_problem::empty_function>(
                Eq(rseq_problem::empty_function{
...
TEST(test_rseq_analyzer_x86, empty_critical_section_is_disallowed)
{
  ...
  EXPECT_THAT(analysis.problems(),
              ElementsAre(VariantWith<rseq_problem::empty_critical_section>(
                Eq(rseq_problem::empty_critical_section{
                  .critical_section_address = elf.symbol("start"),
                  .critical_section_function = "unimportant_function",
                }))));
}
...
TEST(test_rseq_analyzer_x86, label_outside_function_is_disallowed)
{
  ...
  EXPECT_THAT(
    ...
    UnorderedElementsAre(
      VariantWith<rseq_problem::label_outside_function>(
        Eq(rseq_problem::label_outside_function{
          .label_address = start_address,
          .label_function = "test_function",
          .label_kind = rseq_problem::label_outside_function::kind::start,
        })),
      VariantWith<rseq_problem::label_outside_function>(
        Eq(rseq_problem::label_outside_function{
          .label_address = post_commit_address,
          .label_function = "test_function",
          .label_kind = rseq_problem::label_outside_function::kind::post_commit,
        })),
      VariantWith<rseq_problem::label_outside_function>(
        Eq(rseq_problem::label_outside_function{
          .label_address = abort_address,
          .label_function = "test_function",
...
TEST(test_rseq_analyzer_x86, end_of_critical_section_must_follow_beginning)
{
  ...
  EXPECT_THAT(
    ...
    UnorderedElementsAre(VariantWith<rseq_problem::inverted_critical_section>(
      Eq(rseq_problem::inverted_critical_section{
        .critical_section_start_address = elf.symbol("start"),
        .critical_section_post_commit_address = elf.symbol("post_commit"),
        .critical_section_function = "backward_function",
      }))));
}
    ...
    EXPECT_THAT(
      ...
      ElementsAre(
        VariantWith<rseq_problem::stack_pointer_modified>(
          Eq(rseq_problem::stack_pointer_modified{
            .modifying_instruction_address = elf.symbol("push_instruction"),
            .modifying_instruction_string = "pushq %rax",
            .modifying_instruction_function = "test_function",
          })),
        VariantWith<rseq_problem::stack_pointer_modified>(
          Eq(rseq_problem::stack_pointer_modified{
            .modifying_instruction_address = elf.symbol("pop_instruction"),
            .modifying_instruction_string = "popq %rsi",
            .modifying_instruction_function = "test_function",
          }))));
  }
  ...
  for (const auto* instruction : { "retq",
    ...
    EXPECT_THAT(
      ...
      ElementsAre(VariantWith<rseq_problem::stack_pointer_modified>(
        Eq(rseq_problem::stack_pointer_modified{
          .modifying_instruction_address = elf.symbol("bad_instruction"),
          .modifying_instruction_string = instruction,
          .modifying_instruction_function = "fancy_function",
        }))));
  }
}
...
TEST(test_rseq_analyzer_x86, interrupt_in_critical_section_is_disallowed)
{
  for (const auto* instruction : { "syscall", "int3", "int $5" }) {
    ...
    EXPECT_THAT(
      analysis.problems(),
      ElementsAre(
        VariantWith<rseq_problem::interrupt>(Eq(rseq_problem::interrupt{
          .interrupt_instruction_address = elf.symbol("bad_instruction"),
          .interrupt_instruction_string = instruction,
          .interrupt_instruction_function = "test_function",
        }))));
  }
}
...
TEST(test_rseq_analyzer_x86, jumping_into_critical_section_is_disallowed)
{
  ...
  for (const auto* jump_instruction :
    ...
    for (auto jump_location : { before_start, post_commit, post_abort }) {
      ...
      EXPECT_THAT(
        analysis.problems(),
        ElementsAre(VariantWith<
                    rseq_problem::jump_into_critical_section>(testing::AllOf(
          testing::Field(
            &rseq_problem::jump_into_critical_section::jump_instruction_address,
            elf.symbol("bad_instruction")),
          testing::Field(&rseq_problem::jump_into_critical_section::
...
TEST(test_rseq_analyzer_x86, abort_label_must_be_signed)
{
  ...
  EXPECT_THAT(analysis.problems(),
              ElementsAre(VariantWith<rseq_problem::invalid_abort_signature>(
                Eq(rseq_problem::invalid_abort_signature{
                  .signature_address = elf.symbol("abort_signature"),
                  .abort_address = elf.symbol("abort"),
                  .expected_signature = { 0x53_b, 0x30_b, 0x05_b, 0x53_b },
                  .actual_signature = { 0x12_b, 0x34_b, 0x56_b, 0x78_b },
                  .abort_function_name = "broken_function" }))));
}
...
{
  {
    ...
    EXPECT_THAT(analysis.problems(),
                ElementsAre(VariantWith<rseq_problem::invalid_abort_signature>(
                  Eq(rseq_problem::invalid_abort_signature{
                    .signature_address = elf.symbol("abort") - 4,
                    .abort_address = elf.symbol("abort"),
                    .expected_signature = { 0x53_b, 0x30_b, 0x05_b, 0x53_b },
                    .actual_signature = { std::nullopt,
                                          ...
                                          std::nullopt,
                                          std::nullopt },
                    .abort_function_name = "misassembled_function" }))));
  }
     ...
     partially_out_of_bounds_abort_signature_is_disallowed)
{
    ...
    EXPECT_THAT(
      ...
      ElementsAre(VariantWith<rseq_problem::invalid_abort_signature>(
        Eq(rseq_problem::invalid_abort_signature{
          ...
          .abort_address = elf.symbol("abort"),
          .expected_signature = { 0x53_b, 0x30_b, 0x05_b, 0x53_b },
          .actual_signature = { std::nullopt, std::nullopt, 0x53_b, 0x53_b },
          .abort_function_name = "test_function" }))));
  }

  {
    ...
    EXPECT_THAT(
      analysis.problems(),
      UnorderedElementsAre(
        VariantWith<rseq_problem::invalid_abort_signature>(
          Eq(rseq_problem::invalid_abort_signature{
            .signature_address = elf.symbol("abort_signature"),
            .abort_address = abort_address,
            .expected_signature = { 0x53_b, 0x30_b, 0x05_b, 0x53_b },
            .actual_signature = { 0x53_b, 0x53_b, std::nullopt, std::nullopt },
            .abort_function_name = "test_function" })),
        VariantWith<rseq_problem::label_outside_function>(
          testing::Field(&rseq_problem::label_outside_function::label_kind,
...
TEST(test_check_rseq_file, library_with_no_rseq_descriptors_has_problems)
{
  ...
  EXPECT_THAT(analysis.problems(),
              ElementsAre(VariantWith<rseq_problem::no_rseq_descriptors>(
                Eq(rseq_problem::no_rseq_descriptors{
                  .section_name = ".data_cxxtrace_rseq",
                }))));
}
     ...
     rseq_descriptors_with_unallocated_ip_pointers_are_disallowed)
{
  // start, post_commit, and abort addresses are unallocated:
  {
    ...
    EXPECT_THAT(
      ...
      UnorderedElementsAre(VariantWith<rseq_problem::label_outside_function>(
        Eq(rseq_problem::label_outside_function{
          .label_address = elf_so.symbol("my_rseq_function_start"),
          .label_function = "",
          .label_kind = rseq_problem::label_outside_function::kind::start,
        }))));
  ...
  // abort address is unallocated:
  {
    ...
    EXPECT_THAT(
      ...
      UnorderedElementsAre(VariantWith<rseq_problem::label_outside_function>(
        Eq(rseq_problem::label_outside_function{
          .label_address = elf_so.symbol("my_bogus_abort"),
          .label_function = "_my_rseq_function",
          .label_kind = rseq_problem::label_outside_function::kind::abort,
        }))));
  }
}
    ...
    EXPECT_THAT(
      ...
      UnorderedElementsAre(
        VariantWith<rseq_problem::incomplete_rseq_descriptor>(
          Eq(rseq_problem::incomplete_rseq_descriptor{
            .descriptor_address = elf_so.symbol("_my_rseq_descriptor"),
          }))));
  }
}
